...
import com.novpn.data.IncomingAccessKind
import com.novpn.data.IncomingAccessLink
import com.novpn.data.IncomingAccessLinkParser
...
class MainActivity : ComponentActivity() {
    ...
    private var lastHandledIncomingLink = ""
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lastHandledIncomingLink = savedInstanceState?.getString(STATE_LAST_HANDLED_LINK).orEmpty()
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        outState.putString(STATE_LAST_HANDLED_LINK, lastHandledIncomingLink)
    ...
    private fun handleIncomingIntent(intent: Intent?) {
        ...
        val rawLink = incomingUri.toString().trim()
        if (rawLink.isBlank() || rawLink == lastHandledIncomingLink) {
            return
        }
        lastHandledIncomingLink = rawLink

        val parsed = IncomingAccessLinkParser.parse(incomingUri)
        if (parsed == null) {
            Toast.makeText(
                this,
                getString(R.string.incoming_link_unsupported),
                Toast.LENGTH_LONG
            ).show()
            return
        }
        ...
        when (parsed.kind) {
            IncomingAccessKind.INVITE_CODE -> activateIncomingInviteCode(parsed.value)
            IncomingAccessKind.SUBSCRIPTION_URL,
            IncomingAccessKind.VLESS_URL -> importIncomingLink(parsed)
        }
    }

    private fun activateIncomingInviteCode(code: String) {
        ...
        inviteCodeInput.setText(normalizedCode)
        activateInviteCode()
    }

    private fun importIncomingLink(link: IncomingAccessLink) {
        statusTitle.text = getString(R.string.incoming_link_loading)
        statusDetail.text = getString(R.string.incoming_link_loading_detail)

        lifecycleScope.launch {
            runCatching {
                val payloads = when (link.kind) {
                    IncomingAccessKind.SUBSCRIPTION_URL -> listOf(fetchSubscriptionPayload(link.value))
                    IncomingAccessKind.VLESS_URL -> listOf(link.value)
                    IncomingAccessKind.INVITE_CODE -> emptyList()
                }
                viewModel.importProfilePayloads(
                    payloads = payloads,
                    nameHint = "happ-link",
                    apiBaseOverride = link.apiBaseOverride
                )
                ...
                val message = if (importedProfiles.size > 1) {
                    getString(R.string.import_profiles_success_count, importedProfiles.size)
                } else {
                    getString(
                        R.string.import_profile_success,
                        importedProfiles.firstOrNull()?.name ?: viewModel.selectedProfileName()
                    )
                }
                Toast.makeText(
                    this@MainActivity,
    ...
    private suspend fun fetchSubscriptionPayload(subscriptionUrl: String): String {
        return withContext(Dispatchers.IO) {
            val endpoint = URL(subscriptionUrl.trim())
            val connection = (endpoint.openConnection() as HttpURLConnection).apply {
                requestMethod = "GET"
                connectTimeout = 10_000
                readTimeout = 15_000
                setRequestProperty("Accept", "text/plain, application/json;q=0.9, */*;q=0.8")
                setRequestProperty("X-Device-ID", deviceIdentityStore.deviceId())
                setRequestProperty("X-Device-Name", deviceIdentityStore.deviceName())
                setRequestProperty("X-Device-OS", deviceIdentityStore.deviceOS())
                setRequestProperty("X-Device-OS-Version", deviceIdentityStore.deviceOSVersion())
                setRequestProperty("X-Ver-OS", deviceIdentityStore.deviceOSVersion())
            }
            val status = connection.responseCode
            val body = (connection.errorStream ?: connection.inputStream)?.bufferedReader(Charsets.UTF_8)?.use {
                it.readText().trim()
            }.orEmpty()
            if (status !in 200..299) {
                throw IllegalStateException(
                    body.ifBlank { "Subscription import failed with HTTP $status." }
                )
            }
            if (body.isBlank()) {
                throw IllegalStateException("Subscription link returned an empty response.")
            }
            body
        }
    }
    ...
    companion object {
        ...
        private const val STATE_LAST_HANDLED_LINK = "state_last_handled_link"
