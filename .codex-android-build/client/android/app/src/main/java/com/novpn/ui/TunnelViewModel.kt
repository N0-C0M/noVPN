...
import com.novpn.R
import com.novpn.data.AccessRevokedException
...
class TunnelViewModel(application: Application) : AndroidViewModel(application) {
    ...
    suspend fun importProfilePayloads(
        payloads: List<String>,
        nameHint: String = "",
        apiBaseOverride: String = ""
    ): List<AvailableProfile> {
        val importedProfiles = withContext(Dispatchers.IO) {
    ...
    suspend fun refreshGatewayPolicy() {
        ...
        runCatching {
            gatewayPolicyService.fetch(
                ...
                deviceName = deviceIdentityStore.deviceName(),
                deviceOS = deviceIdentityStore.deviceOS(),
                deviceOSVersion = deviceIdentityStore.deviceOSVersion()
            )
        }.onSuccess { snapshot ->
            ...
            if (error is AccessRevokedException) {
                val revokedProfileId = currentProfileId()
                if (revokedProfileId.isNotBlank() && profileRepository.isImportedProfile(revokedProfileId)) {
                    profileRepository.deleteProfile(revokedProfileId)
                    preferences.saveSelectedProfileId(profileRepository.defaultProfileId())
                }
                preferences.saveTrafficQuotaSnapshot(0L, 0L)
                refreshStateFromPreferences()
                _state.update {
                    it.copy(
                        blockedSitesCount = 0,
                        blockedAppsCount = 0,
                        mandatoryNotices = listOf(error.message ?: "Device access was revoked by the server."),
                        trafficUsedBytes = 0L,
